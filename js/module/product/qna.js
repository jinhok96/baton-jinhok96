...
var QNA = {
  getReadData: function (obj, eType) {
    var sHref, pNode, pass_check;
    if (obj != undefined) {
      PARENT = obj;
      sHref = obj.attr('href');
      pNode = obj.parents('tr');
      pass_check = '&pass_check=F';
    } else {
      sHref = PARENT.attr('href');
      pNode = PARENT.parents('tr');
      pass_check = '&pass_check=T';
    }
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        var iProductNo;
        if (aMatchResult) {
          iProductNo = aMatchResult[2];
        } else {
          iProductNo = getQueryString('product_no');
        }
        ...
        if (data.is_secret == true) {
          ...
          if (data.comment != undefined) {
            aHtml.push(
              '<div class="gLeft"><a href="#none" class="btnNormal sizeS" onclick="QNA.comment_view(' +
                data.read['no'] +
                ');">댓글보기 <em>' +
            ...
            aHtml.push(
              '<div class="gRight"><a href="/board/product/modify.html' +
                data.read['param_modify'] +
                '&link_product_no=' +
                iProductNo +
                '" class="btnNormal sizeS">수정</a><a href="#none" class="btnNormal sizeS" onclick="QNA.comment_write(this);">쓰기</a></div>',
            );
          }
          aHtml.push('</div>');
          aHtml.push('</div>');
            ...
            for (var i = 0; data.comment.length > i; i++) {
              ...
              if (data.comment[i]['comment_reply_css'] == undefined) {
                ...
                if (data.comment[i]['comment_reply_display'] == true) {
                  aHtml.push('<div class="ec-base-button">');
                  aHtml.push(
                    '<div class="gLeft"><a href="#none" class="btnNormal sizeS" onclick="QNA.comment_reply_view(' +
                ...
                //댓글의 댓글리스트
                aHtml.push(
                  '<li class="replyArea" style="display:none;" id="' + data.comment[i]['comment_reply_id'] + '">',
                );
                aHtml.push('<div class="commentInfo">');
                aHtml.push(
                  '<strong class="name">' +
